...
class CarController():
  ...
  def update(self, enabled, CS, frame, actuators, visual_alert, pcm_cancel):
    ...
    if (frame % 10) ==0:
      #silence pam
      can_sends.append([0x736, 0, b"\x02\x3E\x80\x00\x00\x00\x00\x00", 1 if CS.CP.isPandaBlack else 0])
      #can_sends.append([0x736, 0, b"\x02\x28\x00\x00\x00\x00\x00\x00", 1 if CS.CP.isPandaBlack else 0])
      ...
      if (frame % 2) == 0:
        ...
        if CS.out.vEgo >= 1:
          ...
          if CS.sappHandshake == 2 and self.sappConfig_last == 20:
            self.sappConfig = 20
            self.angleReq = 1
        print("Handshake:", CS.sappHandshake, "Config:", self.sappConfig_last, "Counter:", self.apaCounter, "PAM Config:", CS.sappConfig)
